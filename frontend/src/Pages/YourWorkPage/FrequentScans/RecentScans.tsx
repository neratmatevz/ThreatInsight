    ...
    const fetchIskanja = async () => {
      ...
      try {
        ...
        const iskanjaList: Iskanje[] = iskanjaQuerySnapshot.docs.map((doc) => ({
          ...
          creationDate: getDuration(
            currentTimestamp,
            doc.data().creationDate.seconds
          ),
          usefulInfoFlags: doc.data().FINALRESULT.usefulInfoFlags,
          vulnerableInfoFlags: doc.data().FINALRESULT.vulnerableInfoFlags,
        }));
          ...
          {iskanja.map((iskanje) => (
            <Col key={iskanje.id}>
              ...
              >
                ...
                {iskanje.usefulInfoFlags !== undefined &&
                iskanje.vulnerableInfoFlags !== undefined &&
                (iskanje.usefulInfoFlags !== 0 ||
                  iskanje.vulnerableInfoFlags !== 0) ? (
                  iskanje.usefulInfoFlags >= iskanje.vulnerableInfoFlags ? (
                    <Badge bg="warning" className="badge-top-right">
                      Useful Informations
                    </Badge>
                  ) : (
                    <Badge bg="danger" className="badge-top-right">
                      Vulnerable Informations
                    </Badge>
                  )
                ) : (
                  <Badge bg="secondary" className="badge-top-right">
                    No Data
                  </Badge>
                )}
              </Card>
