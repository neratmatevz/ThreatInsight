...
const DeleteAccountPage = () => {
  const { user, logout , deleteUserFromCollection} = useAuth();
  ...
  const handleDeleteUser = async (password: string) => {
    try {
      ...
      await deleteUserFromCollection(auth.currentUser.uid)
      logout()
    } catch (error: any) {
  ...
  const handleShowModalDelete = async () => {
    ...
    if(auth.currentUser.providerData.some(provider => provider.providerId === 'google.com')) {
        try {
            await reauthenticateGoogle(auth.currentUser);
            await deleteUserFromCollection(auth.currentUser.uid);
            logout()
        } catch (error : any) {
      ...
      <Row>
        ...
        <Col xs={12} sm={12} md={8} lg={5} style={{ height: "100%" }}>
          <Container fluid className="mt-4">
            <p className="mb-4" style={{ fontSize: "30px", color: "white" }}>
              Delete your account
            </p>
            <p>
              Deleting your ThreatInsight account means you'll lose access to
              all ThreatInsight services, and we'll permanently remove your
            ...
            <PromptForCredentials
              title={
                "Please confirm your password to proceed with deleting your account."
              }
